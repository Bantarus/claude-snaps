...
description: "harness lineage format basics — what `.harness/` stores, how snapshot ids are content-addressable, the distinction between snapshots (composition) and attribution events (sessions/notes), refs vs snapshots, module source kinds (apm/local/builtin), and the v0.5 session-metrics surface + its privacy whitelist. Use when the user asks 'what is .harness/', 'how does harness work', 'what is a snapshot id', 'why didn't a new snapshot land', 'what does HEAD mean in harness', 'what's the difference between a tag and a snapshot', or 'is harness reading my prompts'."
---
